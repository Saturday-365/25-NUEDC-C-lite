...
// 图像色彩空间转化
void Image::colorConvert(Mat& Img,cv::ColorConversionCodes ColorConversionCode)
{
...
// 灰度图像二值化
void Image::BGR::thresholdConvert(Mat& Img)
{
...
// BGR图像通道提取
void Image::BGR::channelSplit(Mat& Img,_BGR_Channel_ BGR_Channel)
{
	vector<Mat> Channel;
	Mat BGR_Img;
...
// 峰值对焦
float Image::BGR::focusPeaking(Mat& Img,_Focus_Core_Size_ FocusCoreSize,float FocusCoreVarianceLimit)
{
	Image Image;
	int Width = Img.cols;
	int Height = Img.rows;
	...
	Image::_Point_ FocusCorePoint;
	vector<Image::_Point_> FocusCorePoints;	// 对焦核左上角坐标
	vector<int> FocusCoreVariances;	// 对焦核方差

	Mat ImgFocus = Img.clone();
	Image.colorConvert(ImgFocus,COLOR_BGRA2GRAY);
	...
	for(int i = 0;i <= Width-FocusCoreSize+1;i++)
	{
		for(int j = 0;j <= Height-FocusCoreSize+1;j++)
		{
			...
			for(unsigned long int m = 0;m < PixelGrayScale.size();m++)
			{
...
// HSV图像亮度直方图均衡化
void Image::HSV::Equalize(cv::Mat& Img)
{
...
// HSV图像二值化
void Image::HSV::thresholdConvert(Mat& Img,Scalar LowLimit,Scalar HighLimit)
{
...
// SOBEL算子边缘检测
void Image::Sobel(Mat& Img,int sobelCore)
{
	Mat ImgX;
	Mat ImgY;
...
// SCHARR算子边缘检测
void Image::Scharr(Mat& Img,int scharrCore)
{
	Mat ImgX;
	Mat ImgY;
...
// CANNY算子边缘检测
void Image::Canny(Mat& Img,int Low,int High,int Size)
{
...
// 高斯模糊
void Image::GaussBlur(Mat& Img,int BlurSize)
{
...
// 图像锐化
void Image::Sharpen(Mat& Img,int BlurSize)
{
	Mat Img_Gauss;
...
// 图形学膨胀
void Image::Dilate(Mat& Img,int CoreSize)
{
...
// 图形学腐蚀
void Image::Erode(Mat& Img,int CoreSize)
{
	Mat Core = getStructuringElement(MORPH_RECT,Size(CoreSize,CoreSize));
	erode(Img,Img,Core);
}
...
// 透视变换
void Image::Unpivot(Mat& Img,Point2f *SrcPoints,Point2f *DstPoints)
{
...
// 图像存储
void Image::Save(Mat Img,const string ImgSave_Path,int ImgNum,_ImgFormat_ ImgFormat)
{
    string ImgSavePath;
    switch(ImgFormat)
    {
...
Mat Image::Show()
{
	...
	for(unsigned long int i = 0;i < ImgSynthesis.size();i++)
	{
...
// 轮廓寻找
void Image::findContours(Mat Img,vector<vector<Point>>& Contours,vector<Vec4i>& Hierarchy,RetrievalModes Mode,ContourApproximationModes Method)
{
...
// 轮廓绘制
void Image::drawContours(Mat& Img,vector<vector<Point>> Contours,vector<Vec4i> Hierarchy,Scalar Color)
{
...
// 轮廓区域最小外接矩形类获取
void Image::findContoursRect(vector<vector<Point>> Contours,vector<RotatedRect>& Rect)
{
	Rect.clear();
	for(unsigned long int i = 0;i < Contours.size();i++)
	{
...
// 轮廓中心点寻找
void Image::findContoursCenter(vector<RotatedRect>& Rect,vector<Point>& Centers)
{
	Centers.clear();
	for(unsigned long int i = 0;i < Rect.size();i++)
	{
...
// 轮廓匹配矩形绘制
void Image::drawContoursRect(Mat& Img,vector<RotatedRect>& Rect,Scalar Color)
{
	// 矩形四角点坐标解析
	Point2f rectLine[4];
	for(long unsigned int i = 0;i < Rect.size();i++)
	{
		Rect[i].points(rectLine);
		for(int k=0;k<4;k++)
		{
		line(Img,rectLine[k],rectLine[(k+1)%4],Color,1);
		}
	}
}

// 图像FPS计算显示
void Image::FPS(Mat& Img,int X,int Y,int FPSperiod)
{
	struct timeval tv;
	...
	cout << "FPS: " << FPS << endl;
	putText(Img,to_string(FPS),cv::Point(X,Y),FONT_HERSHEY_COMPLEX,1,Scalar(255,0,255),1);
}


// 新建画布
void Image::newImg(Mat& Img,int Width,int Height,int ImgType,cv::Scalar Color)
{
