...
void Uart::Send(vector<unsigned char> SendInfos)
{
    for(int i = 0;i < SendInfos.size();i++)
    {
        Base.send(&SendInfos[i],1);
...
void Uart::Receive(unsigned char& ReceiveInfo)
{
...
void Uart::Receive(short int& ReceiveInfo)
{
...
void Uart::Receive(int& ReceiveInfo)
{
...
void Uart::Receive(long int& ReceiveInfo)
{
...
void Uart::Receive(long long int& ReceiveInfo)
{
    unsigned char ReceiveInfoGet[8];
...
// 串口初始化信息获取
void Uart::initInfosGet(const char* DevicePath,_UartDeviceBaudrate_ UartDeviceBaudrate,_ComputerArchtecture_ ComputerArch)
{
...
void Uart::sendInfoAdd(unsigned char data)
{
    SendInfos.push_back(data);
}
void Uart::sendInfoAdd(long long int data)
{
...
void Uart::sendInfoAdd(long int data)
{
...
void Uart::sendInfoAdd(int data)
{
    int* data_p = &data;
    unsigned char* data_add_p = (unsigned char*)data_p;
    for(int i = 0;i < 4;i++)
...
void Uart::sendInfoAdd(short int data)
{
...
// 串口基础库
bool Uart::Base::open(const char* portname, int baudrate, char parity, char databit, char stopbit, char localmode, char outputmode, char inputmode, char synchronizeflag)
{
    // 打开串口
    Uart::Base::pHandle[0] = -1;
    // 以 读写、不阻塞 方式打开
    Uart::Base::pHandle[0] = ::open(portname,O_RDWR|O_NOCTTY|O_NONBLOCK);
...
int Uart::Base::send(const void *buf,int len)
{
    int sendCount = 0;
    if(pHandle[0] != -1)
    {   
...
int Uart::Base::receive(void *buf,int maxlen)
{
