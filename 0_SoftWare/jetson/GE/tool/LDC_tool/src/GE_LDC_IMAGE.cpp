...
int minImages = 74;      // 最少需要采集的图像数
string imgFilePath = "../img/STD/STD/";  // 已经采集的图像路径（图像必须是从0开始标号）
string imgFileFormat = ".jpg";  // 已经采集的图像文件格式
...
int main() {
    // 获取第一帧以确定图像尺寸
    Mat firstFrame;
    firstFrame = imread(imgFilePath+"0"+imgFileFormat);
    if (firstFrame.empty()) {
        cerr << "无法获取相机帧!" << endl;
        return -1;
    }
    ...
    while (true) {
        frame = firstFrame = imread(imgFilePath+to_string(num)+imgFileFormat);
    ...
    if (!imagePoints.empty()) {
        ...
        while (true) {
            Mat undistorted;
            remap(firstFrame, undistorted, map1, map2, INTER_LINEAR);
            imshow("原始图像", firstFrame);
