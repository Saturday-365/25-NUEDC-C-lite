...
PAPER = ROOT / "paper"
TEMPLATE = next(p for p in PAPER.glob("*.docx") if not p.name.startswith("~$"))
MD = next(PAPER.glob("*.md"))
OUT = PAPER / "2315102026 宋嘉诚 基于透视校正的平面目标图形识别与尺寸测量方法研究-排版完成.docx"
...
def add_body_para(doc, text, first_line=True, align=None):
    ...
    add_mixed_text(p, text, bold_prefix=text.startswith("关键词：") or text.startswith("摘  要："))
    return p
